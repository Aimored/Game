...
def get_random_position():
    x=random.randint(10,1490)#23
    y=random.randint(20,810)
    x=x-(x%10)
    y=y-(y%10)
    return (x,y)
...
#Points
newPoint=Canvas(root,width=5, height=5,bg='purple')
...
def move_points():
    ...
    while flag:
        ...
        if (pointsX[0],pointsY[0]) in zip(pointsX[1:],pointsY[1:]):
            flag=False
            Label(root,text="отчислен!!!",font=("Arial",20),foreground='red').place(x=680,y=400)
...
root.bind('<w>', up_fun)
root.bind('<s>', down_fun)
root.bind('<a>', left_fun)
root.bind('<d>', right_fun)
