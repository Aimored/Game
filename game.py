...
def move_krest(krest):
    ...
    if paused:
        return
    # Increase speed based on krest_count, capped at 15
    speed_increment = min(krest_count, 25) + 0.5 # тут
...
def create_and_move_image_2():
    # Create the image at the left edge of the screen
    image_2 = canvas.create_image(0, 540, image=image_image_4)  # Adjust y-coordinate as needed
    move_image_2(image_2)

    # Reschedule the creation of the image every 10 seconds
    window.after(10000, create_and_move_image_2)

def move_image_2(image):
    canvas.move(image, 5, 0)  # Adjust speed as needed
    x, y = canvas.coords(image)
    
    if x < canvas.winfo_width():
        window.after(50, move_image_2, image)  # Adjust refresh rate as needed
    else:
        canvas.delete(image)

# Initialize the movement and creation of image_2
